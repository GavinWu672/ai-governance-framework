#!/usr/bin/env python3
"""
CI/CD pipeline runner for csharp-arch contract validators.

Usage:
  # Run against a single fixture:
  python run_validators.py fixtures/pinvoke_compliant.checks.json

  # Run against all fixtures in a directory:
  python run_validators.py fixtures/

  # Run all fixtures (default):
  python run_validators.py

  # Machine-readable JSON to stdout (text goes to stderr):
...
Enforcement note:
  All rules are currently in advisory mode.
  Hard-stop promotion requires min_evaluations=20 and FP-rate=0.0 (see contract.yaml).
"""
...
_VALIDATOR_MODULES: list[str] = [
    "domain_pinvoke_validator",
]

# Hard-stop rule IDs — none yet; promoted after evaluation threshold is met.
_HARD_STOP_RULES: set[str] = set()


# ── Stub interface (used when governance_tools is not on sys.path) ───────────
...
def _inject_stubs() -> None:
    ...
    when run from the examples directory without installing the package."""
    import types

    if "governance_tools" in sys.modules:
        return  # already available (installed or injected)
...
def _run_fixture(
    ...
    # Normalise: merge top-level keys into payload so validators can read
    # file_path, source_code, domain_roots etc. directly from payload.
...
def _print_report(
    ...
    for r in fixture_result.get("results", []):
        ...
        tag = "HARD-STOP" if hs else ("WARN(advisory)" if not ok else "ok")
        summary = r.get("evidence_summary", "")
        print(f"  [{r['validator']:44s}] {tag:14s}  {summary}", file=out)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="C# arch contract validator runner"
    )
    parser.add_argument(
        "target",
        nargs="?",
    ...
    parser.add_argument(
        "--json", "-j",
        action="store_true",
        help="Print machine-readable JSON to stdout (text goes to stderr)",
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
